...
class Slider {
  constructor(
    ...
    const [back_control, forward_control] = document.getElementById(
      controls_container_id
    ).children;
  ...
  getDynamicStatement() {
    return {
      limit:
        Math.floor(
          this.slider_wrapper.clientWidth / (this.slide_width + this.margins)
        ) || 1,
      from: 0,
  ...
  validateNextTick() {
    ...
    } else if (this.to >= itemsCount) this.invalidateStatement();

    // centring slider regarding to current statemnets;
    this.container.style.transform = `translateX(-${
      this.from * (this.slide_width + this.margins)
    }px)`;
  }
